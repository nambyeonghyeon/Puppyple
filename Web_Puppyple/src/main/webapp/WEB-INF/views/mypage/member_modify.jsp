			...
			<div>
				...
				<div id="mp_wrap">
					<div id="user_info_modify">
						...
						<form action="member_modify_ok.mp" method="get" id="form" name="form" onsubmit="return go_modify()">
							<div id="form_wrap">
								<div id="idForm">
									<label for="member_id">아이디 <span>(수정불가)</span> </label> 
									<input type="text" name="member_id" id="member_id" value="${vo.member_id}" readonly="readonly"/> 
								</div>
								<div id="pwForm">
									<label for="member_pw">비밀번호</label>
									<div class="pw_wrap">
										<input type="password" name="member_pw" id="member_pw" value="${vo.member_pw }" placeholder="4~11자 영문 대 소문자, 숫자, 특수문자를 사용하세요." autofocus/>
										<div class="eye">
											<i class="fa fa-eye"></i>
										</div>
									</div> 
								...
								<div id="pwForm2">
									...
									<input type="password" name="pw_re_check" id="pw_re_check" value="${vo.member_pw }" placeholder="비밀번호를 다시 입력해주세요."/> 
								...
								<div id="nicknameForm">
									...
									<input type="text" name="member_nickname" id="member_nickname" value="${vo.member_nickname}" placeholder="닉네임은 한/영/숫자 2~9자 입니다."/>
								...
								<div id="phoneForm">
									...
									<input type="text" name="member_phone" id="member_phone" value="${vo.member_phone}" maxlength="11" placeholder="- 없이 숫자만 입력해주세요." /> 
								...
								<div id="emailForm">
									...
									<input type="text" name="member_email"	id="member_email" value="${vo.member_email} " placeholder="이메일을 입력해주세요."/> 
							...
							<div id="end">
								...
								<input type="reset" id="reset" value="취소"  onclick=" if(confirm('취소하시겠습니까?')) history.go(-2)"/> 
							</div>
							
						</form>
					</div><!-- user_info_modify -->
				</div><!-- mp_wrqp -->
...
function go_modify() {
	 ...
	 $.ajax({
			...
			, success : function (res) {
				if(res != true && $('#member_nickname').val() == post_nickname){
					check = true;
				}else if(res != true ){
					$('.nic_warning_used').css('display','block').css('color','#b22de4').css('font-size','13px');
	 ...
	 if($('#member_pw').val() == ""){
		 $('.pw_warning_notext').css('display','block').css('color','#b22de4').css('font-size','13px');
		 $('.pw_warning_not').css('display','none');
		 $('#member_pw').focus();
		 return false;
	 }else if(!regPw.test($('#member_pw').val())){
		 $('.pw_warning_notext').css('display','none');
		 $('.pw_warning_not').css('display','block').css('color','#b22de4').css('font-size','13px');
		 $('#member_pw').val("");
		 $('#pw_re_check').val("");
		 $('#member_pw').focus();
		 return false;
	 ...
	 if($('#pw_re_check').val() == ""){
		 $('.pw_warning_notext2').css('display','block').css('color','#b22de4').css('font-size','13px');
		 $('.pw_warning_not2').css('display','none');
		 $('#pw_re_check').focus();
		 return false;
	 }else if( $('#member_pw').val() != $('#pw_re_check').val() ){
		 $('.pw_warning_not2').css('display','block').css('color','#b22de4').css('font-size','13px');
	 ...
	 if($('#member_nickname').val() == ""){
		 $('.nic_warning_notext').css('display','block').css('color','#b22de4').css('font-size','13px');
		 $('.nic_warning_not').css('display','none');
		 $('.nic_warning_used').css('display','none');
		 $('#member_nickname').focus();
		 return false;
	 }else if(!regNickname.test($('#member_nickname').val())){
		 $('.nic_warning_not').css('display','block').css('color','#b22de4').css('font-size','13px');
		 $('.nic_warning_notext').css('display','none');
		 $('.nic_warning_used').css('display','none');
		 $('#member_nickname').val("");
		 $('#member_nickname').focus();
	 ...
	 if($('#member_phone').val() == ""){
		 ...
		 $('.phone_warning_notext').css('display','block').css('color','#b22de4').css('font-size','13px');
		 $('#member_phone').focus();
		 return false;
	 }else if(!regPhone.test($('#member_phone').val())){
		 $('.phone_warning_notext').css('display','none');
		 $('.phone_warning_not').css('display','block').css('color','#b22de4').css('font-size','13px');
		 $('#member_phone').val("");
		 $('#member_phone').focus();
		 return false;
	 }else{
	 ...
	 if($('#member_email').val() == ""){
		 ...
		 $('.email_warning_notext').css('display','block').css('color','#b22de4').css('font-size','13px');
		 $('#member_email').focus();
		 return false;
	 }else if(!email.test($('#member_email').val())){
		 $('.email_warning_notext').css('display','none');
		 $('.email_warning_not').css('display','block').css('color','#b22de4').css('font-size','13px');
		 $('#member_email').val("");
		 $('#member_email').focus();
		 return false;
	 }
