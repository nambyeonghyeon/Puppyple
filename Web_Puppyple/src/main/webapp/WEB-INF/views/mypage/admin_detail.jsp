		...
		<div id="mypage_wrap">
			...
			<section id="layout2">
				<div>
					...
					<div id="mp_wrap">
						<div id="user_info">
							<h5>${vo.member_nickname}님의 회원정보입니다.</h5>
							...
							<c:forEach items="${list }" var="vo">
								...
								<div id="pet_info_table">
									...
									<div id="pet_modifiy_box">
										<a onclick=" if(confirm('정말 삭제하시겠습니까?')) { href='admin_pet_info_delete.mp?pet_id=${vo.pet_id }'} "><input
											type="button" id="pet_delete_btn" class="admin_pet_delete_btn" value="삭제" /></a>
									</div>
									<!-- pet_modifiy_box -->
								</div><!-- pet_info_table -->
						...
						<input type="hidden" name="search" value="${admin_page.search }"/><!-- 검색조건 -->
						<input type="hidden" name="keyword" value="${admin_page.keyword }"/><!-- 검색어 -->
						<input type="hidden" name="curPage" value="${admin_page.curPage }"/><!-- 현재페이지 -->
						<input type="hidden" name="pageList" value="${admin_page.pageList }"/><!-- 페이지당 보여질 목록수 -->
					</form> --%>
