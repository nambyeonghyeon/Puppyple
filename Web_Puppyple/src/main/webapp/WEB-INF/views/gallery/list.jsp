...
<body>
	<div id="content_wrap">
		...
		<div id="community_menubar">
			...
			<div class="title_div">
				<h1 class="title">갤러리<span class="small_title"><span class="small_title_l">|</span> gallery</span></h1>
			</div>
		</div>
					...
					<c:forEach items="${page.list }" var="vo">
						<li>
							...
							<c:if test="${ not empty vo.board_filename }">
								<a style="background-image : url(${vo.board_filepath });" id="preview" href='detail.gal?id=${vo.id }'></a>
								<!-- 미리보기 이미지가 나오는곳 -->
							</c:if> 
							...
							<div class="list_grid_readcnt">
								<i class="fa fa-eye"></i>&nbsp;[${vo.board_readcnt }]
								<span class="list_grid_date">${vo.board_writedate }</span>
							</div>
