...
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions"%>
	<c:forEach items="${list }" var="vo" varStatus="status">
		...
		<div data-id="${vo.id }" class="left" id="comment_inner">${vo.member_nickname }<span id="inner_writerdate">[${vo.writedate }]</span>
			<!-- 로그인한 사용자가 작성한 댓글인 경우 수정/삭제가 가능 -->
			<c:if test="${vo.member_id eq loginInfo.member_id || loginInfo.member_admin eq 'Y'}">
				<span id="inner_modify">
...
$('.btn-modify-save').on('click', function(){
	   ...
	   if( $(this).text() == '수정' ) {
	      ...
	      //줄바꿈 태그 변환
	      var tag = "<textarea style='width:99%; height:90%;'>" + $div.children('.original').html().replace(/<br>/g, '\n') + "</textarea>";
