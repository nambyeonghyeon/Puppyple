...
public class FavoritesController {
	...
	@Autowired private FavoritesServiceImpl service;
		
	//20211222 좋아요 ajax 기능으로 구현 by JP	
	//좋아요 추가 기능 location.href(community)
	@RequestMapping(value = "/favorites_insert", method = {RequestMethod.GET, RequestMethod.POST})
	@ResponseBody
	public void favorites_insert (FavoritesVO vo, Model model, HttpServletRequest req, HttpServletResponse response,HttpSession session) {
	...
	@ResponseBody
	public void favorites_delete_com (FavoritesVO vo, Model model, HttpServletRequest req, HttpServletResponse response,HttpSession session) {
		//1. 일단 좋아요 이미지 클릭시 컨트롤러로 오는지 콘솔창에 확인필요
	...
	//20211222 좋아요 몇명했는지 카운트 하는 기능 by JP
	@RequestMapping("/favorits_count/{pid}")
	public String favorits_count (@PathVariable int pid, Model model, HttpServletRequest req, HttpServletResponse response, HttpSession session) {
		//1. 일단 컨트롤러타는지 확인 필요
